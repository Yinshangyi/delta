...
const households = (sql: SqlClient.SqlClient) => sql`
...
const people = (sql: SqlClient.SqlClient) =>
  Effect.gen(function* () {
    yield* sql`
      CREATE TABLE people (
        id TEXT PRIMARY KEY NOT NULL,
...
const incomeSources = (sql: SqlClient.SqlClient) =>
  Effect.gen(function* () {
    yield* sql`
 ...
 */
const billableDayOverrides = (sql: SqlClient.SqlClient) => sql`
...
export default Effect.gen(function* () {
  ...
  yield* households(sql)
  yield* people(sql)
  yield* incomeSources(sql)
  yield* billableDayOverrides(sql)
})
