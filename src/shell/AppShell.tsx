...
import { SectionIcon } from "@/shell/SectionIcon"
...
export interface AppShellProps {
  ...
  /**
   * The sidebar's footer figure. A slot rather than a value, because the shell
   * is a leaf and the total is an atom read — the container owns that.
   */
 ...
 */
export function AppShell({ current, children, footer }: AppShellProps) {
  return (
    <div className="bg-ground text-ink flex min-h-dvh flex-col md:flex-row">
      ...
      >
        <p className="flex items-baseline gap-2 px-4 pt-4 pb-2 md:px-5 md:pt-6 md:pb-5">
          <span className="text-ink text-base font-semibold tracking-tight">Delta</span>
          {/* The premise, stated once without being asked for (spec §1). */}
        ...
        {footer === undefined ? null : (
          <div className="border-line mt-auto hidden border-t px-5 py-4 md:block">{footer}</div>
        )}
      </nav>
